function [psd, f] = fft_hanningwindow(data, t_start, t_end, channels, fs, window, shift);
...
for i_window = 1:n_window_real
    ...
    % fft_hanningwindow
    w = hann(N,'periodic')';
    temp_window_hanning = w.*temp_window;
    Y = fft(temp_window_hanning, [], 2); % for each row                         
    P2_target = abs(Y/N); 
    ...
    clear temp_window temp_window_hanning Y P2_target P1_target psd_temp_window   
end
